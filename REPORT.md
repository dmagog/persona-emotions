...
**Управляемость.** Стиринг наводит целевую эмоцию устойчиво — все 7 диагоналей
значимы (судья n=56, bootstrap CI), текст остаётся связным (≥82/100).

**Специфичность — условная.** target=argmax только у 4/7 (anger, fear, joy,
...
Вердикт устойчив ко второму судье (qwen-2.5-72b: Pearson 0.90, κ 0.82).

**Над промптингом — нет.** На задаче «навести эмоцию в нейтральном промпте»
простой промптинг строго выигрывает (argmax 7/7 vs 4/7, sadness-протечка +18 vs
+25; по диагонали — 3/7 vs 2/7 у стиринга). Практического выигрыша активационного
стиринга на этой задаче нет.

**SAE-расщепление.** Работает на уровне отбора фич (Jaccard 0.91 → 0.25), но при
декоде обратно в residual направления заново запутываются — при равной силе
SAE-вектор не специфичнее сырого.

**Где стиринг ценен.** В композиционности: дозируемый сплав (joy+sadness — обе
+22..+23 одновременно) и хирургическое вычитание (joy−sadness заостряет joy до
+56, sadness до −34; X−sadness системно гасит «аттрактор» у всех 5 протекающих
эмоций). Это и есть отличительная ниша активационного стиринга — то, чего
инструкцией не получить.
...
contrastive и SAE-feature вектор пересобраны, §8а матрица, §7а top-k и §9
композиция пересчитаны на новом пуле n=56 (8 свежих guilt-held-out промптов) —
все ключевые числа в этом отчёте уже на обновлённых артефактах.
...
Судья + кросс-проверка: mean скоры 66.5–77.4, **905/1000 пар** проходят порог 60
(после пересборки guilt: 115/125 на новых 25 промптах).
...
*Рис. 2. Эффект стиринга монотонно растёт с числом фич, но зависит от эмоции.
...
ими gemma-2-2b рядом с сырым вектором. Ниже — **ранний** сводный замер на n=14 по
энкодеру (для сравнения raw vs SAE-feature по тому же мерилу); достоверная матрица
с CI и судьёй — в §8а.

| (n=14, энкодер) | mean диагональ | target=argmax | протечка в sadness |
|---|---:|---:|---:|
...
полной матрицы убедились, что стиринг по всем 7 эмоциям наводит целевую: на
исходном пуле guilt 32→78 (+46), disgust 10→37, anger 14→41, joy 21→55. «Провал»
guilt/disgust в энкодерной матрице был **артефактом измерения** (GoEmotions их не
видит), а не стиринга. Финальные числа — в §8а ниже (там матрица 7×7 с CI и на
обновлённом пуле, отчасти расходится с этим ранним замером по магнитуде, поскольку
вектор guilt и часть промптов с тех пор изменились). *(`results/judge_steered_gemma_saefeat.md`)*
...
*Рис. 3. Матрица Δ судьи относительно baseline; диагональ обведена. Видно: все
...
*Рис. 4. Те же 56×8 ответы, но оценка энкодером (roberta-go_emotions → 7 ISEAR).
...
слабее shame +18, disgust +8 (на эмоциональных промптах те же векторы давали диагональ
+38..+59, см. §8а).
...
стиринг или почти не работает (coeff 8), или ломает текст (coeff 16+, §9а). То есть
гипотезу «вектор управляет независимо от промпта» в наших условиях подтвердить не
удалось.
