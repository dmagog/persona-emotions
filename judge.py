...
class OpenAiJudge:
    ...
    def __init__(self, model: str, prompt_template: str, eval_type: str = "0_100"):
        self.model = model
        assert eval_type in ["0_100", "0_10", "0_5", "binary", "binary_text"], "eval_type must be one of: 0_100, 0_10, 0_5, binary, binary_text"
        ...
        elif self.eval_type == "0_5":
            self.aggregate_score = self._aggregate_0_5_score
    ...
    async def judge(self, max_retries: int = 5, base_delay: float = 1.0, **kwargs):
        messages = [dict(role='user', content=self.prompt_template.format(**kwargs))]
        if self.eval_type == "binary_text":
            response_text = await self.query_full_text(messages, max_retries, base_delay)
            score = self.aggregate_score(response_text) # aggregate_score is _aggregate_binary_text_score
        else:
            logprobs = await self.logprob_probs(messages, max_retries, base_delay)
    ...
    async def logprob_probs(self, messages, max_retries: int = 5, base_delay: float = 1.0) -> dict:
        ...
        for attempt in range(max_retries):
            ...
            except (oai.RateLimitError, oai.APITimeoutError, oai.InternalServerError) as e:
    ...
    def _aggregate_0_5_score(self, score: dict) -> float:
        #   NOTE: we check for refusals explicitly for BFI inventory.
        #   If "REFUSAL" has the highest probability, we return None.
        #   Otherwise, we aggregate scores from 1-5 (BFI scale).
        if "REFUSAL" in score and score["REFUSAL"] > max(score.get(str(i), 0) for i in range(1, 6)):
            return None
        ...
        for key, val in score.items():
            try:
                ...
                continue
            if int_key < 1 or int_key > 5:  # BFI uses 1-5 scale
                continue
            sum_ += int_key * val
            total += val

        if total < 0.25: # Similar threshold as other methods
            # Failed to aggregate logprobs because total weight on numbers is less than 0.25.
            return None
        return sum_ / total
