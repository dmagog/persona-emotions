...
**5. Композиция.** Арифметика векторов: спеки вида `anger-sadness` наводят разность двух эмоций. Это то, чего промптом не сделать: целевая эмоция растёт, вычитаемая подавляется. Результат `compose.csv`, судейская оценка `compose_judge_wide.csv` (снимается панелью, см. ниже).
...
Порядка 4250 вызовов на модель, около $0.5-0.7 по ценам `llama-3.3-70b`. Кэш судьи переживает перезапуск, но в git не попадает (`*.cache.jsonl` в .gitignore) - после клона пересуживание оплачивается заново.
...
(семейство судьи базовой статьи). Согласие на 11 моделях: Pearson 0.76-0.92.
Фаворитизм измерен: llama-судья к Llama-моделям +1.2 балла к внешним, gemini
к Gemma +4.2. Все файлы `judge_wide_<tag>.csv`, `judge_agreement_<tag>.md`,
