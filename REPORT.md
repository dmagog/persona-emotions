...
Просудили SAE-feature стиреные ответы по всем 7 эмоциям на 56 промптах/условие
(диагональ в `[]`):
...
Сгенерили с pos-инструкцией эмоции (без вектора) на тех же промптах и просудили
(n=56, CI).
...
1. **Управляемость (RQ1) — твёрдо.** Стиринг наводит целевую эмоцию устойчиво: при n=56
   диагональ значима у 6/7 эмоций, текст связный (≥82/100). Это положительный результат.
2. **Специфичность (RQ2) — условная, с честной протечкой.** target=argmax у 6/7,
   все 7 диагоналей значимы (CI без 0), но рядом — значимая вторичная ко-активация
   sadness (+28 [+20,+37]) и близкого негативного кластера. Вердикт **устойчив** ко
   второму судье (qwen-2.5-72b: Pearson 0.90, κ 0.82, argmax совпал 6/7). Энкодер и
   судья при этом дают разную картину; арбитраж «кто ближе к людям» — за человеческой
   разметкой (план 5.3).
