...
Векторы эмоций при этом сильно скоррелированы (косинусы 0.45–0.86, переносится на
Gemma).

Специфичность мерили достоверным образом — судьёй на n=56 с CI. Стиринг наводит
целевую эмоцию значимо по всем 7, **target доминирует у 6/7**, но рядом — значимая
**вторичная** ко-активация sadness (+28 [+20,+37]): целевая сильнее, близкий
негативный кластер тянется следом.

**Граница:** на задаче «навести эмоцию в нейтральном промпте» простой промптинг
не уступает стирингу (и даже специфичнее, 7/7 argmax) — практического выигрыша
активационного стиринга здесь нет.

Оговорка про измеритель: классификатор-энкодер и LLM-судья дают разную картину
специфичности, и ни один пока не объявлен «истиной». Энкодер структурно слеп к
guilt/disgust и шумен на малых выборках, но на остальных 5 эмоциях может быть менее
предвзят — кто ближе к людям, решаем человеческой разметкой. Основные числа здесь
приводим по судье, потому что он покрывает все 7 эмоций.

Отдельный, устойчивый методологический результат: для направлений эмоций проекция
на SAE не расщепляет (Jaccard 0.91), а контраст активаций — расщепляет (0.25) на
уровне **отбора фич**. В поведенческую специфичность стиринга при равной силе это,
однако, не переходит (§7б/W2).

А там, где стиринг всё же обходит промптинг — **композиционность**: можно значимо
смешать эмоции (joy+sadness — обе вверх) и вычесть нежелательную (anger−sadness
гасит протечку в sadness, системно у всех 5 протекающих эмоций), чего инструкцией
не дозируешь. Вердикт **устойчив**: независимый второй судья (qwen-2.5-72b)
согласуется сильно (Pearson 0.90, κ 0.82); а «минимальность» вектора измерена
свипом top-k (~4–8 фич хватает большинству эмоций).
...
*Рис. 1. Все косинусы положительны и высоки (0.58–0.86), что и означает
...
> **Caveat (важно).** Матрица — на n=14 и энкодере. На Gemma при n=56 «аттрактор sadness» исчезает
...
расщепляет. Это разреженный специфичный для эмоции набор фич. *(`results/sae_contrastive_layer12.md`)*
...
*Рис. 3. Эффект стиринга монотонно растёт с числом фич, но эмоция-зависимо.
...
диагонали значимо положительны (target доминирует у 6/7); рядом с диагональю —
вторичные ко-активации в негативном кластере (sadness/guilt/fear/shame), joy стоит
особняком (отрицательные внедиагональные).*
...
эмоции (без вектора) на тех же промптах и просудили (n=56, CI).

Промптинг **сильнее** по диагонали у 5/7 (joy +57 vs +34, guilt +58 vs +49,
anger/disgust выше), **специфичнее** (протечка sadness +18 [9,27] против +28 [20,37]
у стиринга) и даёт **target=argmax 7/7** (против 6/7).

То есть на задаче «навести эмоцию в нейтральном промпте» активационный стиринг через
SAE-фичи **выигрыша над инструкцией не даёт** — даже чуть уступает. Отличительная
ценность стиринга (композиционность, управление без промпта) здесь не проверялась.
...
для всех 5 эмоций с заметной протечкой.

Снижение sadness значимо у **всех 5** (CI без 0): anger +39 [+27,+51], disgust +29
[+17,+41], fear +33 [+21,+45], guilt +41 [+31,+50], shame +34 [+25,+44]. Целевая
эмоция при этом сохраняется значимо у **4/5** (anger +18, disgust +11, fear +20,
shame +34).

**Исключение — guilt:** вычитание гасит и саму вину (+3 [−9,+15], не значимо) —
guilt и sadness в этой модели слиты в один кластер (согласуется с судейской
матрицей guilt~shame~sadness). *(`results/compose_subtract_gemma.md`)*
...
4. **Устойчивый методологический результат:** для направлений эмоций проекция на SAE не
   ...
   метод, не SAE. Это разреженный специфичный для эмоции набор фич. Минимальность измерена
...
- **Судья один и невалидирован (W4) — закрыто по части второго судьи.** Пересудили
  ...
  *Рис. 6. Облако точек жмётся к диагонали y=x, отсюда Pearson 0.90 / Spearman 0.90.
  Хвост на левом нижнем углу — те самые согласованные guilt↔sadness расхождения.*
...
- **Base-SAE на instruct-модели.** GemmaScope SAE обучался на `gemma-2-2b-pt` (base),
  а применяется к активациям `gemma-2-2b-it` (instruct). Инстракт = base + дообучка,
  community-практика допускает такой перенос, но это методологический caveat. Альтернатива
  — инстракт-SAE для gemma-3-it (через SAE Lens), полный перенос пайплайна туда выходит
  за сроки текущей итерации.
- **Компьют — один сетап.** Все основные прогоны — на одной модели (gemma-2-2b-it,
  слой 12, coeff 8, positions=all) и одном GPU (домашний RTX 2070); подбор слоя и
  коэффициента под каждую эмоцию не делали; перенос на другие модели/слои — план
  Фазы 5 (трек Уляны).
...
- **Georgy Mamarin** (соавтор):
  ...
  - Извлечение векторов эмоций и стиринг; матрицы специфичности (раз сырая, два
    SAE-feature, плюс центрированная), bootstrap 95% CI.
  ...
  - Усиление методологии: проверка связности (W5), baseline-промптинг (W7),
    ...
    coeff-свип (§9а), харнесс человеческой разметки (encoder vs judge).
