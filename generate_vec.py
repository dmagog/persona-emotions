...
if __name__ == "__main__":
    ...
    output_files = [
        f"{args.save_dir}/{args.trait}_prompt_avg_diff.pt",
        f"{args.save_dir}/{args.trait}_response_avg_diff.pt",
        f"{args.save_dir}/{args.trait}_prompt_last_diff.pt"
    ]
    if all(os.path.exists(f) for f in output_files):
        print(f"Persona vectors for trait '{args.trait}' already exist in {args.save_dir}. Skipping.")
        exit(0)
