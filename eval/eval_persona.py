...
class Question():
    ...
    async def eval(self, llm, tokenizer, coef, vector=None, layer=None, max_tokens=1000, n_per_question=100, steering_type="last", lora_path=None, judge_max_retries=5, judge_delay=0.1):
        ...
        # Add delay between judge calls to avoid rate limits
...
async def eval_batched(questions, llm, tokenizer, coef, vector=None, layer=None, n_per_question=100, max_concurrent_judges=5, max_tokens=1000, steering_type="last", lora_path=None, judge_delay=0.1, judge_max_retries=5):
    ...
    # Create a semaphore to limit concurrency (reduced default)
    ...
    async def run_with_rate_limiting(task_idx, judge, question_text, answer):
        async with semaphore:
            try:
                ...
                return task_idx, None  # Return None for failed evaluations
    ...
    for i in range(0, len(all_judge_tasks), batch_size):
        ...
        tasks = [run_with_rate_limiting(task_idx, judge, question_text, answer) 
...
def main(model, trait, output_path, coef=0, vector_path=None, layer=None, steering_type="response", max_tokens=1000, n_per_question=10, batch_process=True, max_concurrent_judges=5, persona_instruction_type=None, assistant_name=None, judge_model="gpt-4.1-mini-2025-04-14", version="extract", overwrite=False, judge_delay=0.1, judge_max_retries=5):
    ...
    if batch_process:
        ...
        outputs_list = asyncio.run(eval_batched(questions, llm, tokenizer,coef, vector, layer, n_per_question, max_concurrent_judges, max_tokens, steering_type=steering_type, lora_path=lora_path, judge_delay=judge_delay, judge_max_retries=judge_max_retries))
